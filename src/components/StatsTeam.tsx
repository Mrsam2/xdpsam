...
import XDPSRobot from "@/components/XDPSRobot";
...
export default function StatsTeam() {
  ...
  return (
    <section className="bg-bg-light py-24 md:py-32 border-b border-border-light relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        {/* Title container with 3D Robot */}
        <div className="relative mb-16">
          <h2 className="text-4xl md:text-5xl font-bold tracking-tight text-text-light-primary text-left font-heading relative z-10">
            Scaling Successful Companies
          </h2>

          {/* 3D Robot positioned on the right side of the heading area (hidden on mobile) */}
          <div className="hidden md:block absolute right-[-5%] top-1/2 -translate-y-1/2 w-[350px] h-[350px] z-0">
            <XDPSRobot className="w-full h-full" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
          ...
          >
            <div>
              {/* 4x4 Sparse Grid */}
              <div className="grid grid-cols-4 gap-4 h-auto w-full max-w-[280px] mx-auto mb-8">
              ...
              <p className="text-sm text-text-light-secondary mb-6 leading-relaxed">
                Behind every solution is a team committed to transforming education through innovation. We collaborate to build world-class platforms, nurture talent, and prepare students for the careers of tomorrow
              </p>
            </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            {/* Bottom Right Card: Technology Stack Deck */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6, delay: 0.3 }}
              className="bg-white border border-border-light rounded-2xl flex-grow flex flex-col justify-center items-center shadow-sm hover:shadow-md transition-shadow relative overflow-hidden h-full min-h-[400px]"
            >
              {/* Soft warm blur blob */}
              ...
              <div className="absolute top-0 left-0 w-full p-6 flex justify-between items-center z-20 pointer-events-none">
                <span className="text-[10px] font-black uppercase tracking-widest text-neutral-400">We hire for</span>
              </div>

              <div className="relative z-10 w-full h-full flex flex-col items-center justify-center pt-6">
                <ProductCardDeck />
              </div>
            </motion.div>
          </div>
